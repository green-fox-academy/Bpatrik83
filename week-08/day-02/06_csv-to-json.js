...
let jsonObj = [];
let removedDuplicatesObj = [];

function formattingCSV(csv) {
     return csv.split('\n');
}
function checkDuplicate(obj) {
    let bool = true;
    removedDuplicatesObj.map(object => obj.name != object.name ? bool = true : bool = false);
    bool ? removedDuplicatesObj.push(obj) : '';
}
function createJsonObject(formattedJson) {
    formattedJson.map(item => jsonObj.push({name: `${item.split(';')[0]}`, id: `${item.split(';')[1]}`, colore_code: `${item.split(';')[2]}`}));
    jsonObj.map(object => checkDuplicate(object));
}

createJsonObject(formattingCSV(data));

console.log(removedDuplicatesObj);
